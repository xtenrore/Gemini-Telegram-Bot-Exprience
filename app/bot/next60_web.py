...
const shapes={
 narrow:'M48 5c1-4 5-4 6 0l3 36 28 13c3 2 2 5-1 5l-27-6 3 23 10 8c2 2 1 4-1 4l-15-4-4 11c-1 3-4 3-5 0l-4-11-15 4c-2 0-3-2-1-4l10-8 3-23-27 6c-3 0-4-3-1-5l28-13 3-36z',
 wide:'M48 4c1-4 5-4 6 0l4 34 35 17c3 2 2 5-1 5l-34-9 4 24 13 9c2 2 1 4-1 4l-19-6-4 13c-1 3-4 3-5 0l-4-13-19 6c-2 0-3-2-1-4l13-9 4-24-34 9c-3 0-4-3-1-5l35-17 4-34z',
 business:'M49 5c1-3 4-3 5 0l4 37 24 13c3 2 2 5-1 5l-24-7 4 18 14 8c2 1 1 4-1 4l-16-4 3 10 10 8c2 1 1 4-1 4l-17-6-3 10c-1 2-3 2-4 0l-3-10-17 6c-2 0-3-3-1-4l10-8 3-10-16 4c-2 0-3-3-1-4l14-8 4-18-24 7c-3 0-4-3-1-5l24-13 4-37z',
 prop:'M48 8c1-4 5-4 6 0l2 28 18 10 12-7c3-2 5 1 3 3l-9 9 14 8c3 2 2 5-1 5l-37-8 3 23 11 8c2 2 1 4-1 4l-15-4-4 11c-1 3-4 3-5 0l-4-11-15 4c-2 0-3-2-1-4l11-8 3-23-37 8c-3 0-4-3-1-5l14-8-9-9c-2-2 0-5 3-3l12 7 18-10 2-28z'};
function planePath(type){const t=String(type||'').toUpperCase();if(/A3(3|4|5|8)|A330|A340|A350|A380|B74|B76|B77|B78|B747|B767|B777|B787|B748/.test(t))return shapes.wide;if(/GLF|GLEX|CL3|CL6|C5|C6|C7|LJ|FA7|E50|E55|PRM|H25/.test(t))return shapes.business;if(/AT4|AT7|DH8|DHC|SF3|C208|BE2|PC12/.test(t))return shapes.prop;return shapes.narrow}
function esc(v){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
function section(label,subtitle,source,rows){let out=`<section class="section"><div class="section-head"><b>${label}</b><span>${subtitle}</span><span class="source">${source}</span></div>`;if(!rows.length)return out+'<div class="empty">No current candidates.</div></section>';for(const r of rows){const tone=r.source==='live'?(r.minutes<=15?'live':'dev'):'shadow';const conf=String(r.confidence||'Low').toLowerCase();const timing=r.minutes>30?'window':'to CPA';const status=r.source==='live'?(r.stage||'live geometry'):`history shadow · ${r.history_days||0} day${r.history_days===1?'':'s'}`;out+=`<article class="row"><div class="eta"><strong>${r.minutes}m</strong><small>${timing}</small></div><div class="plane"><svg viewBox="0 0 100 100" aria-hidden="true"><path d="${planePath(r.aircraft_type)}"></path></svg></div><div class="id"><span class="flight">${esc(r.callsign)}</span><span class="model">${esc(r.aircraft_type||'Aircraft')}</span><div class="status"><i class="mark ${tone}"></i>${esc(status)}</div></div><div class="metrics"><div class="dist">${r.closest_km==null?'—':r.closest_km.toFixed(1)+' km'}</div><div class="label">${r.minutes>30?'projected':'closest'}</div><div class="conf ${conf}">${esc(r.confidence)}</div></div></article>`}return out+'</section>'}
