...
This layer sits around the existing trajectory predictor. It never creates an
approach candidate; it only decides whether a live CPA candidate has enough
evidence to be promoted from shadow monitoring to a user-visible alert.

The ensemble is intentionally small and deterministic:
* constant heading
* observed turn continuation
* recent-curvature continuation
* shallow and moderate left/right turn envelopes
* airport-convergence paths for evidence-backed terminal arrivals
* matched historical-route continuation

No AI or paid API participates in these decisions.
"""
...
from app.intelligence.trajectory import (
    KNOTS_TO_KM_S,
    bearing_deg,
    haversine_km,
    project_point,
...
# Calibrated against the 2026-09-18 production false-alert/cancellation traces.
# Qualification is deliberately harder than cancellation so small model changes
# cannot oscillate a live alert on/off.
QUALIFY_SCORE = 0.68
...
# Expected-turn evidence may hold an early alert only for a bounded period. If
# reality does not produce the expected turn, live motion regains authority.
...
def _history_features(
    ...
    *,
    ...
    inconsistent_limit = max(7.5, radius_km * 0.45)
    if inconsistency > inconsistent_limit:
        return 0.0, "dispersed", None

    sims = [route_mod._current_to_history_similarity_km(current, history) for history in histories]
    sims = [value for value in sims if value is not None]
    if not sims:
        return 0.0, "none", None
    similarity = median(sims)
    ...
    for history in histories:
        nearest_index = min(
            ...
            key=lambda index: haversine_km(
                aircraft_lat,
                aircraft_lon,
                history[index].latitude,
                history[index].longitude,
            ),
        )
        future = history[nearest_index:]
        if future:
...
def _simulate_path(
    *,
    ...
    step_s = 5
    points: list[MotionPoint] = [MotionPoint(0.0, lat, lon)]
    ...
    for seconds in range(step_s, horizon_s + 1, step_s):
        ...
        if (
            ...
            and convergence_rate_deg_s is not None
            and seconds > convergence_delay_s
        ):
...
def _motion_paths(*, ac: Any, pred: Any, destination: route_mod.AirportInfo | None, terminal_state: str) -> list[MotionPath]:
    ...
    dest_key = (
        round(float(destination.latitude), 3),
        round(float(destination.longitude), 3),
    ) if destination and destination.latitude is not None and destination.longitude is not None else None
    cache_key = (
    ...
    if terminal and destination is not None and destination.latitude is not None and destination.longitude is not None:
        paths.extend([
            _simulate_path(name="airport_convergence_now", weight=2.00, lat=lat, lon=lon, heading_deg=heading, speed_kts=speed, horizon_s=horizon_s, destination=destination, convergence_rate_deg_s=0.75, convergence_delay_s=0.0),
...
def _evaluate_motion_path(path: MotionPath, *, observer_lat: float, observer_lon: float, radius_km: float) -> HypothesisResult:
    values = [(point.seconds, haversine_km(point.latitude, point.longitude, observer_lat, observer_lon)) for point in path.points]
    seconds, cpa = min(values, key=lambda item: item[1])
...
def assess_candidate(
    *,
    ...
    aircraft_lat = float(ac.latitude)
    aircraft_lon = float(ac.longitude)
    ...
    terminal_score, terminal_state = _terminal_arrival_score(
        ...
        aircraft_lat=aircraft_lat,
        aircraft_lon=aircraft_lon,
        historical_match_score=history_match,
    )
    ...
    if history_cpa is not None and history_match >= 0.35:
        history_weight = 1.5 * history_match if terminal_state != "NOT_TERMINAL" else 0.85 * history_match
        hypotheses.append(HypothesisResult(
            "historical_continuation",
            history_weight,
            history_cpa,
            float(getattr(pred, "time_to_cpa_s", 0.0) or 0.0),
            history_cpa <= alert_radius_km,
        ))

    total_weight = sum(item.weight for item in hypotheses) or 1.0
    pass_weight = sum(item.weight for item in hypotheses if item.enters_radius)
    pass_score = _clamp(pass_weight / total_weight, 0.0, 1.0)

    cpas = [item.cpa_km for item in hypotheses]
    p10 = _percentile(cpas, 0.10)
    p90 = _percentile(cpas, 0.90)
    ...
    return ArrivalAssessment(
        ...
        p10_cpa_km=p10,
        p90_cpa_km=p90,
        prediction_spread_km=max(0.0, p90 - p10),
        expected_turn_state=expected_state,
        expected_turn_direction=expected_direction,
        expected_turn_eta_s=expected_eta,
        airport_path_cpa_km=airport_cpa,
        historical_match_score=history_match,
        historical_cluster=history_cluster,
...
def _encounter_key(ac: Any, user_lat: float, user_lon: float, radius_km: float) -> tuple:
    return (
        str(getattr(ac, "icao24", "")).lower(),
        round(float(user_lat), 4),
        round(float(user_lon), 4),
    ...
    *,
    ...
    if bool(getattr(pred, "already_passed", False)) or str(getattr(pred, "state", "")) == "Passed":
        if moving_away or current_distance_km > encounter.observed_min_km + 0.3:
            ...
            encounter.passed_mono = now_mono
    ...
    if bool(getattr(pred, "stale", False)):
        encounter.positive_count = max(0, encounter.positive_count - 1)
    ...
    strong_live_pass = assessment.pass_score >= threshold
    if assessment.expected_turn_state == "TURN_DID_NOT_OCCUR":
        expected_turn_holds = False
        threshold = max(QUALIFY_SCORE, threshold - 0.08)
        required = min(required, 2)
        strong_live_pass = assessment.pass_score >= threshold

    if active:
        if assessment.pass_score < CANCEL_SCORE or expected_turn_holds:
            return True, "ACTIVE_BELOW_CANCEL_HYSTERESIS" if not expected_turn_holds else assessment.expected_turn_state, encounter.positive_count
...
async def evaluate_route_v42(
    ...
    *,
    ...
    directly_inside = not bool(getattr(pred, "stale", False)) and float(getattr(pred, "current_distance_km", math.inf)) <= alert_radius_km
    if directly_inside:
        suppress = False
    ...
    result = RouteGateResultV42(
        ...
        reason=base.reason if suppress and qualification_state == "SHADOW_ROUTE_EVIDENCE" else f"v4.2 {qualification_state}: ensemble pass score {assessment.pass_score:.3f}",
        history_days=base.history_days,
        similar_days=base.similar_days,
        ...
        ensemble_pass_score=assessment.pass_score,
    ...
    logger.debug(
        "v42_decision %s",
        json.dumps({
            "aircraft": getattr(ac, "icao24", ""),
            "flight": getattr(ac, "callsign", ""),
...
def install_route_guard_v42() -> None:
    ...
    logger.info(
        ...
        QUALIFY_SCORE,
        TERMINAL_QUALIFY_SCORE,
        CANCEL_SCORE,
        QUALIFY_CONFIRMATIONS,
