...
def client():
    with patch("app.main.connect_db", new_callable=AsyncMock), \
         ...
         patch("app.main.start_slack_socket", new_callable=AsyncMock, return_value=None), \
         patch("app.main.stop_slack_socket", new_callable=AsyncMock):
...
def test_telegram_webhook_is_gone(client):
    resp = client.post("/webhook", json={})
    assert resp.status_code == 410
    assert "Slack Socket Mode" in resp.text
