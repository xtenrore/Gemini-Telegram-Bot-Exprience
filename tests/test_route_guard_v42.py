...
def assessment(*, score, live_score=None, terminal="NOT_TERMINAL", expected="NONE", airport_cpa=None):
    return ArrivalAssessment(
        ...
        live_pass_score=score if live_score is None else live_score,
        hypotheses=(
...
def test_case_a_terminal_arrival_points_at_user_but_airport_paths_turn_away():
    result = assess_candidate(
        ac=ac(), pred=pred(), destination=IST, route_plausible=True,
        current_samples=history_to_current(), historical_paths=[],
...
def test_case_b_expected_turn_failure_releases_shadow_when_live_ensemble_is_strong():
    ...
    # Overall score is deliberately low because old airport/history futures say
    # miss; once the expected turn deadline is missed, live motion must dominate.
    release = assessment(
        score=0.34, live_score=0.86, terminal="TERMINAL_ARRIVAL",
        expected="TURN_DID_NOT_OCCUR", airport_cpa=20.0,
    )
    ...
    assert second[0] is False and second[1] == "QUALIFIED_PASS"


def test_case_c_known_ist_destination_does_not_globally_suppress_real_close_pass():
    encounter = EncounterState(100.0, 100.0)
    live = pred(current=7.5, cpa=2.0, cpa_s=55.0)
    real_pass = assessment(
        score=0.92, terminal="TERMINAL_ARRIVAL",
        expected="TURN_DID_NOT_OCCUR", airport_cpa=5.5,
    )
    _, state, _ = _apply_qualification(
...
def test_case_d_destination_missing_history_plus_descent_degrades_gracefully():
    historic = [[
        (41.0, 29.30), (41.0, 29.24), (41.0, 29.18), (41.0, 29.12),
        (41.05, 29.02), (41.12, 28.92),
    ]]
    result = assess_candidate(
        ac=ac(), pred=pred(), destination=None, route_plausible=False,
        current_samples=history_to_current(), historical_paths=historic,
        observer_lat=USER[0], observer_lon=USER[1], alert_radius_km=RADIUS,
        encounter=EncounterState(100.0, 100.0), now_mono=100.0,
    )
...
def test_case_f_stale_adsb_is_uncertainty_not_turn_confirmation():
    ...
    suppressed, state, confirmations = _apply_qualification(
        base=base(),
        assessment=assessment(score=0.9, terminal="TERMINAL_ARRIVAL", expected="EXPECTED_TURN_PENDING", airport_cpa=20.0),
        encounter=encounter, pred=pred(stale=True), current_distance_km=16.0,
...
def test_case_g_passed_encounter_cannot_reopen_from_bogus_inbound_vector():
    encounter = EncounterState(
        100.0, 100.0, qualified=True, passed=True, passed_mono=100.0,
        observed_min_km=4.0, previous_distance_km=10.0,
    )
    suppressed, state, _ = _apply_qualification(
        base=base(), assessment=assessment(score=0.95), encounter=encounter,
        pred=pred(current=12.0, cpa=1.0, cpa_s=80.0, trend=-0.05),
...
def test_case_h_expected_turn_evidence_cancels_shadow_before_notification():
    ...
    second = _apply_qualification(
        base=base(),
        assessment=assessment(score=0.30, terminal="TERMINAL_ARRIVAL", expected="EXPECTED_TURN_PENDING", airport_cpa=24.0),
...
def test_case_i_hysteresis_keeps_qualified_pass_above_cancel_threshold():
    ...
    cancelled = _apply_qualification(
        base=base(), assessment=assessment(score=CANCEL_SCORE - 0.10),
        encounter=encounter, pred=pred(), current_distance_km=13.0,
        radius_km=RADIUS, active=True, now_mono=115.0,
    )
...
def test_case_j_global_motion_paths_are_reused_across_users_and_cache_is_bounded():
    first = _motion_paths(ac=ac(), pred=pred(), destination=IST, terminal_state="TERMINAL_ARRIVAL")
    second = _motion_paths(ac=ac(), pred=pred(), destination=IST, terminal_state="TERMINAL_ARRIVAL")
    assert first is second
    assert len(first) <= 9
    assert len(_path_cache) <= 128


def test_temporal_confirmation_resets_after_long_data_gap():
    encounter = EncounterState(100.0, 100.0, positive_count=1)
    suppressed, state, confirmations = _apply_qualification(
        base=base(), assessment=assessment(score=0.90), encounter=encounter, pred=pred(),
        current_distance_km=15.0, radius_km=RADIUS, active=False, now_mono=130.0,
    )
    assert suppressed is True
    assert state == "TRAJECTORY_CONFIRMING"
    assert confirmations == 1


def test_turn_started_becomes_turn_confirmed_when_airport_heading_is_reached():
    encounter = EncounterState(100.0, 100.0, expected_turn_started=True)
    # Put the aircraft on the direct bearing to IST after the turn has begun.
    aircraft = ac(lat=41.0, lon=29.15)
    from app.intelligence.trajectory import bearing_deg
    aircraft.heading = bearing_deg(aircraft.latitude, aircraft.longitude, IST.latitude, IST.longitude)
    result = assess_candidate(
        ac=aircraft, pred=pred(turn=-0.4), destination=IST, route_plausible=True,
        current_samples=history_to_current(), historical_paths=[],
        observer_lat=USER[0], observer_lon=USER[1], alert_radius_km=RADIUS,
        encounter=encounter, now_mono=110.0,
    )
    assert result.expected_turn_state == "TURN_CONFIRMED"
    assert encounter.expected_turn_confirmed is True


def test_turn_did_not_occur_uses_live_score_not_stale_airport_hypotheses():
    encounter = EncounterState(100.0, 100.0)
    mixed = assessment(
        score=0.25, live_score=0.84, terminal="TERMINAL_ARRIVAL",
        expected="TURN_DID_NOT_OCCUR", airport_cpa=30.0,
    )
    first = _apply_qualification(
        base=base(True, "matching recent routes turn/stay outside the observer alert radius"),
        assessment=mixed, encounter=encounter, pred=pred(cpa_s=80.0),
        current_distance_km=14.0, radius_km=RADIUS, active=False, now_mono=190.0,
    )
    second = _apply_qualification(
        base=base(True, "matching recent routes turn/stay outside the observer alert radius"),
        assessment=mixed, encounter=encounter, pred=pred(cpa_s=75.0),
        current_distance_km=12.5, radius_km=RADIUS, active=False, now_mono=195.0,
    )
    assert first[1] == "TRAJECTORY_CONFIRMING"
