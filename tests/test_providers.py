...
def test_parse_adsb_response():
    """Verify parsing standard ADSB.lol / ADSB.fi JSON structure."""
    raw_data = {
        "ac": [
            {
                "hex": "40082c",
...
def test_provider_manager_merge_and_type_cache():
    """Verify deduplication across providers and type cache enrichment."""
    pm = ProviderManager()

    # Provider A reports ac with type B738
    ac1 = NormalizedAircraft(
        icao24="400111",
        callsign="BAW1",
        latitude=51.5,
        longitude=-0.1,
    ...
    # Provider B reports same ac with missing type
    ac2 = NormalizedAircraft(
        icao24="400111",
        callsign="BAW1",
        latitude=51.501,
        longitude=-0.101,
